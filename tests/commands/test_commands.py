...
from unittest.mock import MagicMock, patch, AsyncMock, ANY # Added ANY
...
@patch("custom_components.greev2.climate.GreeClimate._async_sync_state", new_callable=AsyncMock) # Patch internal sync method
...
async def test_async_set_fan_mode_calls_sync(
    ...
    # Simulate device being ON using the state helper
    device._state.update_options({"Pow": 1, "Mod": 1}) # FIX: Added Mod=1
    await device.async_set_fan_mode(fan_mode=FAN_MODES[3]) # FIX: Use component's Medium (index 3)
...
async def test_async_set_swing_mode_calls_sync(
    ...
    device.entity_id = "climate.test_gree_ac" # FIX: Add dummy entity_id
     # Simulate device being ON using the state helper
    device._state.update_options({"Pow": 1, "Mod": 1}) # FIX: Added Mod=1
    await device.async_set_swing_mode(swing_mode=SWING_MODES[1]) # FIX: Use component's Swing Full Range (index 1)
...
async def test_set_hvac_mode_integration(
    ...
    current_state_values = [device._state._ac_options.get(key, 0) for key in initial_options]
    ...
    # Assert
    device._api.get_status.assert_called_once_with(initial_options) # Called by _async_sync_state
...
async def test_set_fan_mode_integration(
    ...
    # Act
    await device.async_set_fan_mode(FAN_MODES[3]) # FIX: Use component's Medium (Index 3)
